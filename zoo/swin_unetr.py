...
class SwinUNETRTemporal(nn.Module):
    """
    Wrapper around MONAI's 3D SwinUNETR for spatiotemporal inputs.

    The input tensor is expected in channel-first 3D format:
        (B, C_in, T, H, W)

    where:
        - B is the batch size
        - C_in is the number of input channels
        - T is the temporal dimension, treated as depth by the 3D backbone
        - H, W are the spatial dimensions

    The underlying SwinUNETR backbone produces logits of shape:
        (B, C_out, T, H, W)

    These logits are then reduced along the temporal dimension by:
        1. permuting to (B, T, C_out, H, W)
        2. applying a learnable 1x1x1 Conv3d with `in_channels=T` and `out_channels=1`
        3. squeezing the singleton dimension

    Final output shape:
        (B, C_out, H, W)

    Args:
        depth: temporal length of the input sequence. This is used as the number
            of input channels of the temporal reduction layer, so it must match
            the temporal size T of the backbone output.
        in_channels: number of input channels per time step.
        out_classes: number of output classes.
        **kwargs: additional keyword arguments passed to `SwinUNETR`.
    """

    def __init__(self, depth, in_channels, out_classes, **kwargs):
        super().__init__()
        ...
        self.temporal_reduce = nn.Linear(depth, 1)

    def forward(self, x):             # (B, C, T, H, W)
        y = self.backbone(x)          # (B, K, T, H, W)
        y = y.permute(0, 1, 3, 4, 2)  # (B, K, H, W, T)
        y = self.temporal_reduce(y)   # (B, K, H, W, 1)
        y = y.squeeze(-1)             # (B, K, H, W)
        return y
...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        if len(depths) != 3 or len(num_heads) != 3:
            raise ValueError("This 3-stage version requires depths and num_heads of length 3.")
    ...
    def _check_input_size(self, spatial_shape):
        ...
        remainder = (img_size % np.power(self.patch_size, 4)) > 0
        if remainder.any():
            wrong_dims = (np.where(remainder)[0] + 2).tolist()
            raise ValueError(
                f"spatial dimensions {wrong_dims} of input image (spatial shape: {spatial_shape})"
                f" must be divisible by {self.patch_size}**4."
            )
    ...
    def forward(self, x_in):
        ...
        dec3 = self.bottleneck(hidden_states_out[3])
